...
class IFS() {
    ...
    def deleteAffine(a: Affine) {
      affines = affines.filterNot(_ == a)
    }
    ...
    def paintEditor(g: Graphics2D) {
      affines.foreach(_.paint(g))
    }
    ...
    def paintPreview(g: Graphics2D) {
      ...
      // warm up before you start rendering
      var p = Point(.5, .5)
      for(i <- 1 to burnInSteps) p = takeStep(p)
      // real thing
